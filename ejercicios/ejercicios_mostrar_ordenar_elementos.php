...
class mod_ejercicios_mostrar_ejercicio_ordenar_elementos extends moodleform_mod {
    ...
    function creando_ejercicio(&$mform, $id, $npreguntas, $tipoorigen, $tiporespuesta) {
        ...
        $npreg = $ejercicioGeneral->get('numpreg');
        $creador = $ejercicioGeneral->get('id_creador');
        $tipo_origen = $ejercicioGeneral->get('tipoarchivopregunta');
        ...
        //Opciones para crear la ordenacion 
        $opciones = '<br><br><center><span>Seleccion el tipo de Ordenación Elementos que desea realizar </span><select id="opcionOrden" name="opcionOrden"> <option value="1">Párrafos de un texto</option> <option value="2">Frases de un párrafo</option> <option value="3">Palabras dentro de una frase</option> <option value="4">Desordenar letras de las palabras</option> </select></center>';
        $mform->addElement('html', $opciones);
        $boton = '<center><input type="button" name="botonOrden" id="botonOrden" value="Comenzar" onclick="tipoOrden()" /> </center>';
...
//        $mform->addElement('html', $guardarTextoHuecos);
        ...
        $titulo = '</br><h3>' . get_string('OE_texto', 'ejercicios') . '1' . '</h3>';
        $mform->addElement('html', $titulo);
        
        //textarea para introducir el texto a desordenar
        $mform->addElement('textarea', 'original1', get_string("OE_add_frase", 'ejercicios'), 'wrap="virtual" rows="10" cols="130"');
        //Se añade un boton para desordenar 
        $divBoton = '<div name="divBoton" id="divBoton" class="divBoton">';
        $divBoton.= '<br><center><input type="button" name="desordenar1" id="desordenar1" value="' . get_string('OE_boton', 'ejercicios') . '" onclick="desordenar(this.id)" /> </center><br>';
        $divBoton.= '</div>';
        $mform->addElement('html', $divBoton);
        //textarea para mostrar al profesor como queda el texto desordenado
        $mform->addElement('textarea', 'pregunta1', get_string("OE_desorden", 'ejercicios'), 'wrap="virtual" rows="10" cols="130" disabled="disabled"');
...
//        $mform->addGroup($buttonarray, 'botones', '', array(' '), false);
    ...
    function mostrar_con_permisos(&$mform, $id, $npreguntas, $id_ejercicio, $tipo_origen, $tipo_respuesta, $ejercicios_leido) {
        /**
